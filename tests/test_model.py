...
torch_tensor = torch.tensor([[0, 6, 100013, 49125, 27683, 1344, 2]])
paddle_tensor = paddle.to_tensor([[0, 6, 100013, 49125, 27683, 1344, 2]])


my_model = InfoXLMModel.from_pretrained(os.path.join(ROOT, "model_checkpoints/converted_paddle"))
...
def test_forward_is_ok():
    ...
    assert (np.abs(lhs[0] - rhs[0]).max() < 2e-3)

    assert (np.abs(lhs2[0] - rhs2[0]).max() < 2e-3)
